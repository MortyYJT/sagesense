...
Last updated: Saturday, 22 August 2026, 01:47 AEST
...
- Android project has a current 21,454,010-byte debug RC with Kotlin, Compose, Room, DataStore and manual dependency injection. Its SHA-256 is `aabcc3d26a80d61f6db22fea051b3b1b2bffe3357dd077edcbcbf725dfad1f9a`.
...
- A versioned startup migration now rewrites retained pre-hardening events through
  the same persistence privacy filter. An `install -r` emulator test verified
  hash removal, password/phone redaction, origin-only links and restart idempotence.
